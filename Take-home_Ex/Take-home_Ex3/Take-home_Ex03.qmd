...
resale_tidy <- resale_tidy %>%
  mutate(
    min_storey = as.numeric(str_extract(storey_range, "^[0-9]+")),  # Extract minimum value in each range
    storey_scale = ((min_storey - 1) %/% 3) + 1               # Convert to scale: 1 for 1-3, 2 for 4-6, etc.
  ) %>%
  select(-min_storey)  # Optional: Drop min_storey if not needed
...
We will change the order of columns in the resulting dataset, `resale_regression` to make it easier to read too.
...
![](images/Screenshot 2024-11-09 160250.png)
...
From the above, we infer that the optimal bandwidth that we must use to create this model is .
...
From the above models and predictions created, we infer that the model performs better on the test data from 2023 compared to predicting prices for July to September 2024, however the model is viable in both cases, as shown by the plot in 3.5.1.4 and 3.5.2.4.
...
There has been increasing demand due to a wide variety of reasons:
